...
import com.equalinformation.shipping.poc.pi.components.ProfilePreferencesWindow;
 ...
 */
public class ShippingMenu extends CustomComponent {
    ...
    public ShippingMenu() {
        ...
        // There's only one ShippingMenu per UI so this doesn't need to be
        // unregistered from the UI-scoped ShippingEventBus.
    ...
    private Component buildTitle() {
        Label logo = new Label("Shipping <strong>Dashboard</strong>",
                ContentMode.HTML);
    ...
    private Component buildUserMenu() {
        ...
        settingsItem.addItem("Sign Out", new MenuBar.Command() {
            @Override
            public void menuSelected(final MenuBar.MenuItem selectedItem) {
                ShippingEventBus.post(new ShippingEvent.UserLoggedOutEvent());
            }
        });
        return settings;
    }
    ...
    private Component buildMenuItems() {
        ...
        for (final ShippingViewType view : ShippingViewType.values()) {
            Component menuItemComponent = new ValoMenuItemButton(view);

            if (view == ShippingViewType.VIEW2) {
                ...
                reports.setDropHandler(new DropHandler() {
                    ...
                    public void drop(final DragAndDropEvent event) {
                        UI.getCurrent()
                                .getNavigator()
                                .navigateTo(
                                        ShippingViewType.VIEW2.getViewName());
            ...
            if (view == ShippingViewType.VIEW1) {
    ...
    @Subscribe
    public void postViewChange(final ShippingEvent.PostViewChangeEvent event) {
    ...
    public void updateNotificationsCount(
            final ShippingEvent.NotificationsCountUpdatedEvent event) {
        int unreadNotificationsCount = ShippingUI.getDataProvider()
    ...
    @Subscribe
    public void updateReportsCount(final ShippingEvent.ReportsCountUpdatedEvent event) {
        reportsBadge.setValue(String.valueOf(event.getCount()));
        reportsBadge.setVisible(event.getCount() > 0);
    }

    @Subscribe
    public void updateUserName(final ShippingEvent.ProfileUpdatedEvent event) {
    ...
    public final class ValoMenuItemButton extends Button {
        ...
        private final ShippingViewType view;

        public ValoMenuItemButton(final ShippingViewType view) {
            this.view = view;
